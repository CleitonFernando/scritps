...
encerramento(){
        if [ -e "$TEMP/bkperro.pid" ]
                then
                        cat $LOGFILE >> $LOG
                        mail -s "BACKUP ERRO - Cliente: [$CLIENTE] Servidor: $SRV" seuemail $EMAILCLIENTE < $LOGFILE
                        ...
                        cat $LOGFILE > $LOG
                        mkdir -p $HOME/1 2> /dev/null ; mv $BKPTEMP/* /$HOME/1/ 2> /dev/null
                        mail -s "BACKUP OK - Cliente: [$CLIENTE] Servidor: $SRV" seuemail $EMAILCLIENTE < $LOGFILE
        fi
}
